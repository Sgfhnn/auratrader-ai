          ...
          <div className="bg-dark-800 border border-gray-700/50 rounded-xl p-4">
            ...
            <div className="flex flex-wrap items-center gap-4 md:gap-6 mb-4">
              <div>
                ...
                <div className="text-lg md:text-xl font-mono font-bold text-red">{p.bid?.toFixed(5) || '—'}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Ask</div>
                <div className="text-lg md:text-xl font-mono font-bold text-green">{p.ask?.toFixed(5) || '—'}</div>
              </div>
              <div className="ml-auto flex gap-2">
                <button onClick={() => handleTrade('BUY')} disabled={loading}
                  className="px-3 md:px-5 py-2 bg-green hover:bg-green-muted text-white font-semibold rounded-lg text-sm disabled:opacity-50">
                  BUY
                </button>
                <button onClick={() => handleTrade('SELL')} disabled={loading}
                  className="px-3 md:px-5 py-2 bg-red hover:bg-red-muted text-white font-semibold rounded-lg text-sm disabled:opacity-50">
                  SELL
                </button>
              </div>
            </div>
